    ...
    comments: [
      {
        ...
        text: "Could be a nitrogen deficiency. Have you tested your soil pH?",
      },
      {
        id: 2,
        author: "hydro_guru",
        text: "Also check for root rot, yellow leaves can be a sign of overwatering.",
      },
    ],
  },
  {
    ...
    comments: [
      {
        id: 3,
        author: "pixel_farmer_82",
        text: "Looks great! What nutrient solution are you using?",
      },
    ],
  },
  {
    ...
    comments: [
      {
        ...
        text: "Neem oil is a classic for a reason. Effective and non-toxic.",
      },
      {
        id: 5,
        author: "agri_tech_pro",
        text: "Try companion planting. Marigolds deter a lot of common pests.",
      },
    ],
  },
  {
    ...
    comments: [
      {
        id: 6,
        author: "earth_worm_jim",
        text: "Good tip, thanks! My tomatoes are looking a bit spotty.",
      },
    ],
  },
];
...
// 1. Navigation Bar Component (MODIFIED)
const AgroShieldNavbar = () => {
  // Step 2: Convert navItems to an array of objects with paths
  const navItems = [
    { name: "Home", path: "/" },
  ...
  return (
    <nav className="bg-[#0a0a0a] border-b-2 border-t-2 border-[#333] p-2 flex justify-center items-center text-sm md:text-base">
      <div className="flex items-center space-x-2 md:space-x-4">
        {navItems.map((item) => (
          // Step 3: Replace <a> with NavLink
          <NavLink
            key={item.path}
            to={item.path}
            // NavLink provides an `isActive` property in its render prop
            className={({ isActive }) =>
              `px-3 py-1 relative transition-all duration-200 ${
                isActive ? "text-[#00ff00]" : "text-[#aaa] hover:text-[#fff]"
              }`
            }
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <span className="absolute -left-1 top-1.5 animate-flicker text-[#00ff00]">
                    &gt;
                  </span>
                )}
                {item.name}
                {isActive && (
                  <span className="absolute -right-1 top-1.5 animate-flicker text-[#00ff00]">
                    &lt;
                  </span>
                )}
                <div
                  className={`absolute bottom-0 left-0 w-full h-[2px] ${
                    isActive ? "bg-[#00ff00]" : "bg-transparent"
                  }`}
                ></div>
              </>
            )}
...
// 2. Main Post Card Component (MODIFIED)
  ...
  <div className="flex flex-col bg-[#111] p-2 border-2 border-[#222] hover:border-[#444] transition-colors duration-300">
    <div className="flex">
      ...
      <div className="flex flex-col items-center w-10 text-[#777] pr-2">
        ...
        >
          <DownvoteIcon className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="flex-1">
        <div className="flex items-center mb-1">
          {post.flair && (
            <span
              className={`text-[10px] ${post.flair.color} text-white px-1.5 py-0.5 mr-2 font-bold`}
            >
              {post.flair.text}
            </span>
          )}
          {/* Step 4: Make post title a Link */}
          <Link
            to={`/post/${post.id}`}
            className="text-white text-base md:text-lg font-bold hover:text-green-400"
          >
            {post.title}
          </Link>
        </div>
        <p className="text-xs text-[#777]">
          {/* Step 5: Make author a Link */}
          submitted {post.time} by{" "}
          <Link
            to={`/user/${post.author}`}
  ...
  return (
    <aside className="w-full lg:w-72 flex-shrink-0 space-y-4">
      <div className="border-2 border-[#333] bg-[#0a0a0a] p-3">
        <h3 className="text-[#00ff00] font-bold mb-2 border-b-2 border-[#333] pb-1">
          ABOUT AGROSHIELD_COMM
        </h3>
        <p className="text-xs text-[#ccc]">
          A community for modern farmers, agronomists, and tech enthusiasts to
          discuss crop health, sustainable practices, and innovative solutions.
        </p>
        <div className="text-xs text-[#ccc] mt-3">
          <p>
            <span className="font-bold text-white">1.2k</span> Members
          </p>
          <p>
            <span className="font-bold text-white">42</span> Online
          </p>
        </div>
      </div>
      <div className="border-2 border-[#333] bg-[#0a0a0a] p-3">
      ...
      <div className="border-2 border-[#333] bg-[#0a0a0a] p-3">
        ...
        <ul className="space-y-2 text-xs text-blue-400">
          {topContributors.map((user) => (
            <li key={user} className="hover:underline cursor-pointer">
              {user}
            </li>
          ))}
        </ul>
...
export default function CommunityPage() {
  ...
  const handleSort = (sortType) => {
    ...
    let sortedPosts = [...initialPosts]; // Use initialPosts to reset sort order

    if (sortType === "Hot" || sortType === "Top") {
      sortedPosts.sort((a, b) => b.votes - a.votes);
    } else if (sortType === "New") {
      // A simple reverse might not be accurate for "New", but works for this mock data
  ...
  const handleVote = (id, direction) => {
    ...
    // Re-sort after voting to maintain the correct order
    if (activeSort === "Hot" || activeSort === "Top") {
      ...
      <div className="font-pixel min-h-screen crt-background">
        ...
        <main className="p-4 md:p-8 max-w-7xl mx-auto text-white">
          ...
          <header className="mb-6 text-center">
            ...
            <p className="text-[#aaa] text-sm md:text-base mt-1">
              Your daily terminal for agricultural intel.
            </p>
          </header>
                ...
                {sortOptions.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1 text-sm border-2 transition-colors ${
                        isActive
                          ? "bg-green-900 border-green-500 text-white"
                          : "bg-[#222] border-transparent text-[#999] hover:border-[#555]"
                      }`}
                    >
